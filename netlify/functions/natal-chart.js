...
function buildMailHTML(nom, date_naissance, heure_naissance, lieu, geo, chart) {
...
<div style="max-width:660px;margin:0 auto;background:#0d0d0d;border:1px solid rgba(212,175,55,.2);padding:40px;">
  ...
  <div style="border-top:1px solid rgba(212,175,55,.08);padding-top:16px;text-align:center;">
...
exports.handler = async function(event) {
  ...
  try {
    const { nom, date_naissance, heure_naissance, lieu_naissance, email } = JSON.parse(event.body);
    ...
    const chart = await fetchNatalChart(year, month, day, localHour, localMin, geo.lat, geo.lon);
    const html  = buildMailHTML(nom, date_naissance, heure_naissance, lieu_naissance, geo, chart);
